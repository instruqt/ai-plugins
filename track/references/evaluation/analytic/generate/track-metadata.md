...
Score 4: Layout matches content — code-heavy tracks use smaller sidebar (30), reading-heavy use larger (40-45). Theme is set. Loading messages are configured for tracks with long setup times.
Score 5: Every lab_config setting is deliberately chosen and justified by the content. Loading messages are specific to what is being provisioned. Layout sidebar size is tuned per challenge where needed.
Score 3: Lab_config is partially configured — layout is set but loading messages and theme are defaults.
Score 2: Lab_config is mostly defaults. Layout does not match the content type.
...
Score 4: Pausable is set appropriately (yes for long/multi-session tracks, no for short). skipping_enabled reflects whether challenges are independent or sequential. Settings are coherent with each other.
Score 5: All behavioral settings are deliberately configured with the delivery context in mind. Pausable tracks have appropriate TTL. show_timer is set based on whether time pressure adds value or stress.
...
Score 2: Settings are inconsistent — e.g., pausable with a short TTL, or skipping enabled when challenges have hard dependencies.
